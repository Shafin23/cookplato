...
const Favourite = () => {
  ...
  const filteredCooks = allcooks?.filter((cook) => {
    if (
      selectedCategory &&
      cook.dishes.some((dish) => dish.category === selectedCategory)
    ) {
      return true; // If selected category matches and exists, include the cook
    }
    if (
      searchTerm &&
      cook.dishes.some((dish) =>
  ...
  return (
    <div className="mx-auto w-[90vw] pb-20">
      <div>
        <h1 className="text-5xl font-normal mb-7">
          Find your favourite food or cook
        </h1>
        ...
        <div className="mb-20 grid grid-cols-1 grid-rows-4 md:grid-rows-1 md:grid-cols-4 gap-8">
          ...
          <div className="dropdown">
            <div
              tabIndex={0}
              role="button"
              className="px-3 py-3 w-full border border-gray-300 text-gray-500 flex justify-between items-center"
            >
              <span>Search a category</span> <IoIosArrowDown />
            </div>
            <ul
              tabIndex={0}
            ...
            >
              ...
              <li>
                <button onClick={() => handleCategoryChange("Appetizers")}>
                  Appetizers
                </button>
              </li>
              <li>
              ...
              <li>
                <button
                  onClick={() => handleCategoryChange("Meat Preperations")}
                >
                  Meat Preperations
                </button>
              </li>
              <li>
                <button
                  onClick={() => handleCategoryChange("Fish and Seafood")}
                >
                  Fish and Seafood
                </button>
              </li>
              <li>
                <button onClick={() => handleCategoryChange("Supe")}>
                  Supe
                </button>
              </li>
              <li>
                <button onClick={() => handleCategoryChange("Dough Delights")}>
                  Dough Delights
                </button>
              </li>
              <li>
                <button onClick={() => handleCategoryChange("other")}>
                  other
                </button>
              </li>
              <li>
                <button onClick={() => handleCategoryChange("BBQ")}>BBQ</button>
              </li>
              <li>
                <button
                  onClick={() => handleCategoryChange("Traditional Food")}
                >
                  Tradistional Food
                </button>
              </li>
        ...
        {isLoading ? (
          ...
          >
            {filteredCooks.map((item) => (
              <div
                key={item._id}
                className="card me-0 md:me-8 bg-base-100 shadow-xl rounded-none my-10"
              >
                <figure>
                  <img
                    src={
                ...
                <div className="card-body">
                  ...
                  <div className="card-actions flex justify-between items-center">
                    <button
                      onClick={() => handlePersonalProfile(item._id)}
                      className="btn rounded-md bg-amber-400 hover:bg-amber-500 transition-all duration-300 w-[45%]"
                    >
                      Visit
                    </button>
                    <button className="btn w-[45%] rounded-md transition-all">
                      Follow
                    </button>
                  </div>
                </div>
